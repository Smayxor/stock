...
pnl = 0
mostDown = 0
fileList = [x for x in dp.pullLogFileList() if ((ticker=='SPX') ^ ('SPY' in x))].sort()
print(f'Testing days{fileList}')
firstFile = fileList[0]
firstDay = str(datetime.datetime.strptime(firstDay.replace('-datalog.json',''), '%Y-%m-%d') - datetime.timedelta(days=30)).split(' ')[0]
lastFile = fileList[-1]
...
for file in fileList:
	...
	totalCandles = 0
	while x > 14:
		if candles[x]['date'] == end: 
			print(f'Found day {end}')
			break
		x -= 1
	continue
	...
	lowestPrice = 9999
	highestPrice = 0
	lowestPriceTime = "6:00"
	highestPriceTime = "6:00"
	...
	for time in gexData:
		...
		price = gexData[time]['price']
		if price > highestPrice:
			highestPrice = price
			highestPriceTime = minute
		if price < lowestPrice:
			lowestPrice = price
			lowestPriceTime = minute
		...
		if (callEntered == 1):
			for strike in gexData[time]['data']:
				if (strike[dp.GEX_STRIKE] == callStrike) :
					...
					if ((strike[dp.GEX_CALL_BID] >= lowestCallPrice * 3) or (x == lenData)):  # or (minute == "12:59")
		...
		if (putEntered == 1):
			for strike in gexData[time]['data']:
				if (strike[dp.GEX_STRIKE] == putStrike):
					...
					if ((strike[dp.GEX_PUT_BID] >= lowestPutPrice * 3) or (x == lenData)):
	...
	print( f'{end} - {lowestPriceTime} LowestPrice {lowestPrice} - {highestPriceTime} HighestPrice {highestPrice}' )

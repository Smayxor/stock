...
def updateCanvas():
    ...
    for i in range(0, len(options), 2) :
        ...
        #y = 400 + ((call.Strike - SpotPrice) * 4)
        if 0 < (call.Strike - SpotPrice) < 5 :
            draw.line([0, y, 400, y], fill='purple', width=1)
            draw.text((400,y), text=f'{SpotPrice}', fill='purple', font=font, anchor='la')

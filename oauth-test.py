...
if serverMode:
    ...
    font = ImageFont.truetype("Arimo-Regular.ttf", 28, encoding="unic") #Place font file in same folder, or use supply path if needed in Linux
...
def addStrike(strike, volume, oi, delta, gamma, vega, price, volatility, call, itm, bid, days):
    global ExpectedMove, GEX, DEX, VEX, Volas, OIS
    try:
...
#Draw the chart
    ...
    for strikes in sorted(GEX):
        x += 15
        drawText(x, 235, txt=('\n'.join(str(strikes).replace('.0', ''))), color=("#0FF" if (x % 10 == 0) else "#3F3"))
        if (OIS[strikes] != 0): drawRect(x, 30, x + 9, 30 + ((abs(OIS[strikes]) / maxOIS) * 50), color="#00f", border='')
